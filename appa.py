...
while True:
    ...
    if len(buffer) > window_size:
        ...
        # Apply Savitzky-Golay filtering for smooth signal
        if len(buffer) > 51:  # Ensure window size is valid
            smoothed_signal = signal.savgol_filter(buffer, 51, 3)
        else:
            smoothed_signal = buffer
        ...
        if len(filtered_freqs) > 0:
            ...
            bpm = peak_freq * 60
            cv2.putText(frame, f'BPM: {int(bpm)}', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)  # Blue color
    
    # Create second window for signal monitoring
    signal_frame = np.zeros((300, 500, 3), dtype=np.uint8)
    cv2.putText(signal_frame, 'Signal Monitoring', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)  # Blue color
    if len(buffer) > 1:
        min_val, max_val = min(buffer), max(buffer)
        if max_val > min_val:  # Avoid division by zero
            normalized_buffer = np.interp(buffer, (min_val, max_val), (50, 250))
            for i in range(1, len(normalized_buffer)):
                cv2.line(signal_frame, (i-1, int(300 - normalized_buffer[i-1])), (i, int(300 - normalized_buffer[i])), (255, 0, 0), 2)  # Blue color
